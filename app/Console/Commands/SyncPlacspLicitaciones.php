...
class SyncPlacspLicitaciones extends Command
{
    protected $signature = 'placsp:sync-licitaciones
    ...
    protected $description = 'Sincroniza licitaciones desde el feed Atom de PLACSP';
    ...
    public function handle(ContratoImporter $importer, PlacspEntryParser $parser): int
    {
        ...
        $this->info($dryRun ? 'Modo DRY-RUN: no se insertaran datos.' : 'Sincronizando licitaciones PLACSP...');
        ...
        if (! $dryRun) {
            ...
            if (! $fuente) {
                $this->error('Fuente de datos "placsp-licitaciones" no encontrada. Ejecuta: php artisan db:seed');
        ...
        // Configurar reader
        $feedUrl = config('contratacion.placsp.licitaciones_feed');
        ...
        if (! $dryRun && $fuenteDatosId) {
            ...
            FuenteDatos::where('slug', 'placsp-licitaciones')
    ...
    private function processEntry(
        ...
        try {
            ...
            // Flags específicos de licitaciones
            $data['es_menor'] = false;
    ...
    private function showSummary(int $duration, bool $dryRun): void
    {
        ...
        $this->newLine();
        $this->info("{$prefix}Resumen sincronizacion licitaciones PLACSP:");
        $this->table(
            ['Metrica', 'Valor'],
            [
